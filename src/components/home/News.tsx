...
export function News() {
  return (
    <section id="news" className="py-16 md:py-24 bg-background relative">
          ...
          {caseStudies.map((study) => (
            <Card key={study.id} className="group border-none shadow-lg rounded-[6%] overflow-hidden bg-white transition-all hover:shadow-2xl hover:-translate-y-2">
              <CardHeader className="p-0 relative h-64 overflow-hidden">
                ...
                <Badge className="absolute top-4 left-4 bg-primary text-white border-none font-bold px-3 py-1 rounded-[6%] text-[10px]">
                  {study.category}
                </Badge>
                <div className="absolute bottom-4 left-4 text-white transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 flex items-center gap-2">
                  <Target className="w-4 h-4 text-accent" />
                  <span className="font-bold text-xs">{study.result}</span>
                </div>
              </CardHeader>
              <CardContent className="p-6 md:p-8">
                <div className="flex items-center gap-4 text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-3">
                ...
                <h4 className="text-xl md:text-2xl font-headline font-bold mb-3 group-hover:text-primary transition-colors line-clamp-2">
                  {study.title}
                </h4>
              ...
              <CardFooter className="p-6 md:p-8 pt-0">
                <Button variant="link" className="p-0 text-primary font-bold group/btn flex items-center gap-2 hover:no-underline text-sm">
